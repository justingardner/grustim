...
%
function [task myscreen] = fbsear_fixStairInitTask(myscreen)
...
if ~isfield(fixStimulus,'threshold') fixStimulus.threshold = 0; end
...
if ~isfield(fixStimulus,'stairStepSize') fixStimulus.stairStepSize = 0; end
...
response = find(task.thistrial.buttonState) == stimulus.live.correctButton;
response = response(1);
