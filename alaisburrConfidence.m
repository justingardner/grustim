...
%
%
function myscreen = alaisburrConfidence(varargin)
...
% trial: Fixation + Stim1 (.015s/.0015s) + ISI (.5s) + Stim2 (.015s/.0015s) + Resp + ITI
   task{1}{1}.segmin = [1 stimulus.stimDur stimulus.ISI stimulus.stimDur inf inf 1];
   task{1}{1}.segmax = [1 stimulus.stimDur stimulus.ISI stimulus.stimDur inf inf 1];
   task{1}{1}.getResponse = [0 0 0 0 1 1 0];
else
    % trial: Fixation + Stim1 (.015s/.0015s) + ISI (.5s) + Stim2 (.015s/.0015s) + Resp + ITI
...
task{1}{1}.randVars.calculated.initialConfidence = nan;
...
elseif task.thistrial.thisseg == 6
  ...
  task.thistrial.confidence = randsample([0: 0.1: 1], 1);
