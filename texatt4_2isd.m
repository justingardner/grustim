...
% add arguments later
scan = 1;
getArgs(varargin,{'scan=0','noeye=0', 'nTrials=90'}, 'verbose=1');
stimulus.scan = scan;
...
task{1}{1}.parameter.leftTopImgClass = 1:length(stimulus.imageNames);
task{1}{1}.parameter.rightTopImgClass = 1:length(stimulus.imageNames);
task{1}{1}.parameter.leftBottomImgClass = 1:length(stimulus.imageNames);
task{1}{1}.parameter.rightBottomImgClass = 1:length(stimulus.imageNames);
...
task{1}{1}.randVars.uniform.leftTopSample1 = [0,1,2]; % 0 = original, 1 = synth sample 1, 2 = synth sample2
task{1}{1}.randVars.uniform.leftBottomSample1 = [0,1,2]; % 0 = original, 1 = synth sample 1, 2 = synth sample2
task{1}{1}.randVars.uniform.rightTopSample1 = [0,1,2]; 
task{1}{1}.randVars.uniform.rightBottomSample1 = [0,1,2]; 
task{1}{1}.randVars.calculated.leftTopSample2 = NaN;
task{1}{1}.randVars.calculated.leftBottomSample2 = NaN;
task{1}{1}.randVars.calculated.rightTopSample2 = NaN;
task{1}{1}.randVars.calculated.rightBottomSample2 = NaN;
...
for li = 1:length(locations)
	loc = locations{li};
	imgClass = stimulus.imageNames{task.thistrial.(sprintf('%sImgClass', loc))};
	if task.thistrial.(sprintf('%sSame', loc)) == 0
	  other_samples = setdiff([0,1,2], task.thistrial.(sprintf('%sSample1', loc)));
	  task.thistrial.(sprintf('%sSample2', loc)) = other_samples(randsample(length(other_samples),1));
	else
	  task.thistrial.(sprintf('%sSample2', loc))= task.thistrial.(sprintf('%sSample1', loc));
	end
	stimulus.live.(sprintf('%sStim1', loc)) = stimulus.images.(sprintf('%s_s%i', imgClass, task.thistrial.(sprintf('%sSample1', loc))));
	stimulus.live.(sprintf('%sStim2', loc)) = stimulus.images.(sprintf('%s_s%i', imgClass, task.thistrial.(sprintf('%sSample2', loc))));
end
