% gloloadapt.m
%
%      usage: [myscreen,task,stimulus]=gloloadapt(stimulus)
%         by: justin gardner
%       date: 09/17/08
%    purpose: gloloadapt program
%             
%       e.g.: myscreen = gloloadapt
%
function [myscreen, task, stimulus] = gloloadapt(randstate)
...
% set the adaptation direction
if isempty(randstate)
  stimulus.adaptdir = [-1 1];
elseif isodd(randstate)
  stimulus.adaptdir = [-1 1];
else
  stimulus.adaptdir = [1 -1];
end
...
stimulus.patches.probeFlickerMaxContrast = 0.25;
...
task{2}{1}.parameterCode.localdir = [0 0];
task{2}{1}.parameterCode.globaldir = -inf;
task{2}{1}.parameterCode.orientation = [1 1];
task{2}{1}.random = 0;
task{2}{1}.seglen = [15];
task{2}{1}.seglen = [1.5];
...
% adaptation stimulus
task{2}{2}.numTrials = 15;
task{2}{2}.numTrials = 1;
task{2}{2}.parameter.dirnum = [1;2];
task{2}{2}.parameterCode.localdir = [0 0];
task{2}{2}.parameterCode.globaldir = stimulus.adaptdir;
task{2}{2}.parameterCode.orientation = [1 1];
task{2}{2}.random = 0;
task{2}{2}.seglen = [3.75 0.25];
task{2}{2}.timeInVols = 0;
task{2}{2}.waitForBacktick = 0;

% trials
task{2}{3}.parameter.dirnum = [1:3;1:3];
task{2}{3}.parameterCode.localdir =  [0 0 0 0];
task{2}{3}.parameterCode.globaldir = [-1 1 -inf];
task{2}{3}.parameterCode.orientation = ones(1,3);
task{2}{3}.random = 1;
% set up segments of trials
%rampDownTime = stimulus.patches.pdur/myscreen.framesPerSecond;
%task{2}{3}.segmin = [4.8 3.5];
%task{2}{3}.segmax = [4.8 9.5];
task{2}{3}.segmin = [3 1 4 1];
task{2}{3}.segmax = [3 1 12 1];
task{2}{3}.synchToVol = [0 0 0 1];
task{2}{3}.timeInVols = 0;
task{2}{3}.waitForBacktick = 0;
...
% same for probe
stimulus.patches.probeFlickerContrast = stimulus.patches.probeFlickerMaxContrast * cos(flickerphases);
% and multily by envelope
stimulus.patches.probeFlickerContrast = stimulus.patches.probeFlickerContrast.*stimulus.patches.contrast;

% now add one contrast and phase for 0 contrast
stimulus.patches.flickerContrast(stimulus.patches.pdur+1) = 0;
stimulus.patches.probeFlickerContrast(stimulus.patches.pdur+1) = 0;
...
for contrastNum = 1:length(stimulus.patches.contrast)
  ...
  % probe stimulus
  mprobe = stimulus.patches.probeFlickerContrast(contrastNum)*m;
  % multiply by contrast
  ...
  if ~debugGrating
    ...
    stimulus.patches.stim{contrastNum,4} = mglCreateTexture(myscreen.grayIndex+myscreen.inc*mprobe);
    stimulus.patches.stim{contrastNum,5} = mglCreateTexture(myscreen.grayIndex-myscreen.inc*mprobe);
  end
...
stimulus.patches.thislocaldir(1:2,1:stimulus.patches.numcols,1:stimulus.patches.numrows) = 2;
stimulus.patches.desiredLocaldir(1:2,1:stimulus.patches.numcols,1:stimulus.patches.numrows) = 2;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function myscreen = setPatchDir(myscreen,task)
...
%    stimulus.patches.desiredContrastSeqNum(side,colnum,:) = (1:stimulus.patches.toffset:stimulus.patches.numrows*stimulus.patches.toffset)+toffsetPhase;
  ...
  % global dir set to -inf turn off patches
  elseif stimulus.patches.globaldir(side) == -inf
    % set all patches to have desired contrast sequence number of -inf, so
    % that they will stay gray
    stimulus.patches.desiredContrastSeqNum(side,:,:) = -inf;
  end
...
%    stimulus.patches.desiredMotiontype(side,:,:) = 2 + (rand(stimulus.patches.numrows,stimulus.patches.numcols)>0.5);
    ...
    if (task.thistrial.thisphase == 3) && (task.thistrial.thisseg == 1)
      stimulus.patches.desiredMotiontype(side,:) = 4;
      stimulus.patches.motiontype(side,:) = 4;
    else
      stimulus.patches.desiredMotiontype(side,:) = 2;
      stimulus.patches.motiontype(side,:) = 2;
    end
  else
...
% offset the contrast sequence at the very beginning now, when
...
if (task.thistrial.thisseg == 1)
  stimulus.patches.contrastSeqNum(:) = inf;
  ...
  myscreen = setPatchDir(myscreen,task);
elseif (task.thistrial.thisseg == 3)
  stimulus.patches.contrastSeqNum(:) = inf;
  % set local and global directions
  stimulus.patches.localdir = [0 0];
  stimulus.patches.globaldir = stimulus.adaptdir;
  stimulus.patches.orientation = [1 1];
  % display settings
  disp(sprintf('%i:orient=%i local=%i global=%i | %i:orient=%i local=%i global=%i',task.thistrial.dirnum(1),stimulus.patches.orientations(1),stimulus.patches.localdir(1),stimulus.patches.globaldir(1),task.thistrial.dirnum(2),stimulus.patches.orientation(2),stimulus.patches.localdir(2),stimulus.patches.globaldir(2)));
  % actually set the patch directions
  myscreen = setPatchDir(myscreen,task);
