...
%
%        $Id:$ 
%      usage: facesdt(<'faceDir=faceDir'>,<'placeDir=placeDir'>)
%         by: justin gardner
%       date: 06/14/10
%    purpose: get psychometric function for faces vs places
%
function retval = facesdt(varargin)
...
imageDir = [];
stimfile = [];
getArgs(varargin,{'faceDir=faces','imageDir=~/proj/faceplace/FaceHouseStim','stimfile=[]'});
...
% load images
stimulus = myInitStimulus(stimulus,myscreen,faceDir,imageDir);
...
task{1}.parameter.scrambleFactor = 0.4:0.2:0.8;
task{1}.randVars.uniform.presentAbsent = [0 1];
task{1}.numBlocks = 150;
...
if ~isfile(filename)
  disp(sprintf('(facesdt) Could not find file %s',filename));
  return
end

stimfile = load(filename);
if ~isfield(stimfile,'myscreen') || ~isfield(stimfile,'task')
  disp(sprintf('(facesdt) File %s does not contain myscreen/task',filename));
  return
end
...
if ~isfield(task{1},'trialnum') || (task{1}.trialnum <= length(task{1}.parameter.scrambleFactor))
  disp(sprintf('(facesdt:dispPsychometricFunction) Not enough data to display'));
  return
end
...
if task.thistrial.thisseg == 1
  ...
  % if this is an absent trial, then we will just scamble all phases, only
  % if the signal is present should we scramble a percentage of the phases
  if task.thistrial.presentAbsent
    scrambledPhases = scrambledPhases(1:floor(faceIm.n*(1-task.thistrial.scrambleFactor)));
  end    
  % scramble the phases
...
myscreen.flushMode = 1;
disp(sprintf('presentAbsent: %i scrambleFactor: %f choice: %i',task.thistrial.presentAbsent,task.thistrial.scrambleFactor,task.thistrial.whichButton));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function to init the dot stimulus
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function stimulus = myInitStimulus(stimulus,myscreen,faceDir,imageDir)
