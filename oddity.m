function [ myscreen ] = oddity( varargin )
%
% TEXTURE ODDBALL TASK
%  Oddball task using textures 
%
%  Usage: oddity(varargin)
...
if ~isempty(mglGetSID) && isdir(sprintf('~/data/oddity/%s',mglGetSID))
  % Directory exists, check for a stimfile
  files = dir(sprintf('~/data/oddity/%s/1*mat',mglGetSID));

  if length(files) >= 1
    fname = files(end).name;
    
    s = load(sprintf('~/data/oddity/%s/%s',mglGetSID,fname));
    stimulus.counter = s.stimulus.counter + 1;
    clear s;
    disp(sprintf('(oddity) Data file: %s loaded.',fname));
  end
end
disp(sprintf('(oddity) This is run #%i',stimulus.counter));
...
%%% Stimulus Variables
trial_info = load('~/proj/oddity/trial_info.mat');
stimulus.trial_info = trial_info;
stimulus.trial_indices = trial_info.trial_indices;
...
% Trial parameters
task{1}.parameter.trial_index = stimulus.trial_indices; % Randomly assign trial index on each trial
...
for i = 1:length(images)
  x = strsplit(images{i}, '.');
  filename = sprintf('%s/%s', stim_dir, images{i});
  smp = imread(filename);
  stims.(sprintf('image%s', x{1})) = genTexFromIm(smp, stimulus.live.mask);
  disppercent(i / length(images));
end
disppercent(inf);
...
% let the user know
disp(sprintf('(oddity) Starting run number: %i.',stimulus.counter));
