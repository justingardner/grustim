...
stimulus.imNames = {'fronds', 'monarchs', 'paisley', 'rocks', 'zebras'};
...
stimulus.rewardProbs = stimulus.rewardRange(1):((stimulus.rewardRange(2)-stimulus.rewardRange(1))/4):stimulus.rewardRange(2);
...
for i = 1:2
  ...
  elseif task.thistrial.thisseg == stimulus.seg.feedback
    if task.thistrial.rewarded
      mglBltTexture(mglText('Nice! You win $1 :)'), [0,2]);
      upFix(stimulus, stimulus.colors.green);
    else
      mglBltTexture(mglText('Sorry! No reward :('), [0,2]);
