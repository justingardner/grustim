...
%
%             Note from Dan: this is just a version that doesn't actually
%             draw anything to the screen.
%
function [task myscreen] = gruFixStairInitTask_afmap(myscreen)
...
% init the staircase
if ~isfield(fixStimulus,'staircase')
    if fixStimulus.stairUseLevitt
        fixStimulus.staircase = doStaircase('init','upDown','nup',fixStimulus.stairUp,...
            ...
            'initialStepsize',fixStimulus.stairStepSize,'testType=levitt','minThreshold',0,'maxThreshold',1);
    elseif fixStimulus.stairUsePest
        fixStimulus.staircase = doStaircase('init','upDown','nup',fixStimulus.stairUp,...
            'ndown',fixStimulus.stairDown,'initialThreshold',fixStimulus.threshold,...
            'initialStepsize',fixStimulus.stairStepSize,'testType=pest','minThreshold',0,'maxThreshold',1);
    else
        fixStimulus.staircase = doStaircase('init','upDown','nup',fixStimulus.stairUp,...
            'ndown',fixStimulus.stairDown,'initialThreshold',fixStimulus.threshold,...
            'initialStepsize',fixStimulus.stairStepSize,'minThreshold',0,'maxThreshold',1);
    end
else
    disp('Re-using staircase');
end
