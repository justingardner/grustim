function [ myscreen ] = texSeg( varargin )
%
% TEXTURE ODDBALL TASK
%  Oddball task using textures 
%
%  Usage: texSeg(varargin)
...
if ~isempty(mglGetSID) && isdir(sprintf('~/data/texSeg/%s',mglGetSID))
  % Directory exists, check for a stimfile
  files = dir(sprintf('~/data/texSeg/%s/1*mat',mglGetSID));

  if length(files) >= 1
    fname = files(end).name;
    
    s = load(sprintf('~/data/texSeg/%s/%s',mglGetSID,fname));
    stimulus.counter = s.stimulus.counter + 1;
    clear s;
    disp(sprintf('(texSeg) Data file: %s loaded.',fname));
  end
end
disp(sprintf('(texSeg) This is run #%i',stimulus.counter));
...
% Trial parameters
task{1}.parameter.targetInterval = 1:2;
...
task{1}.randVars.uniform.textureFamily = 1:length(stimulus.imNames);
task{1}.randVars.uniform.targetEccentricity = 2:stimulus.imSize;
task{1}.randVars.uniform.targetPolarAngle = 0:pi/4:2*pi;

% Task variables to keep track of the status of each trial
task{1}.randVars.calculated.detected = 0;
task{1}.randVars.calculated.dead = 0;
task{1}.randVars.calculated.correct = NaN;

%% Preload images
stimulus.nSamples = 1; % preload 3 samples of each kind.
...
% let the user know
disp(sprintf('(texSeg) Starting run number: %i.',stimulus.counter));
...
imName = stimulus.imNames{task.thistrial.textureFamily};
...
% get the files list
files = dir(fullfile(sprintf('~/data/texSeg/%s/19*stim*.mat',mglGetSID)));

count = 1; 
data = struct('response', [], 'reaction_time', [], 'nTrials', 0, 'nValTrials', 0, 'accByRuns', []);
for fi = 1:length(files)
  load(fullfile(sprintf('~/data/texSeg/%s/%s',mglGetSID,files(fi).name)));
