...
%       
function myscreen = cogneuro_workingmemory_z(scan)
...
task{1}.segmin = [.4 .25 stimulus.delayInterval-.5 .25 2.5];
task{1}.segmax = [.4 .25 stimulus.delayInterval+.5 .25 2.5];
...
if task.thistrial.thisseg == 1
  ...
  % get the threshold value to test (choose from the correct staircase)
  [task.thistrial.orientationThreshold stimulus.s{task.thistrial.cue}] = doStaircase('testValue',stimulus.s{task.thistrial.cue});
  ...
  % set the match (if this cue 2 then just do the task left or right of 45
  if task.thistrial.cue == 1
    stimulus.matchOrientation = 45+task.thistrial.orientationThreshold*task.thistrial.clockwiseCounterclockwise;
  else
    stimulus.matchOrientation = task.thistrial.orientation(1)+task.thistrial.orientationJitter(1)+task.thistrial.orientationThreshold*task.thistrial.clockwiseCounterclockwise;
  end
  
  % display what we have selected
  disp(sprintf('====================='));
  if task.thistrial.cue ==1
    disp(sprintf('Trial %i: Axis (test value: %0.2f)',task.trialnum,task.thistrial.orientationThreshold));
  else
    disp(sprintf('Trial %i: Remember (test value: %0.2f)',task.trialnum,task.thistrial.orientationThreshold));
  end
  disp(sprintf('Trial %i: Orientation: %0.1f + %0.1f: %f',task.trialnum,task.thistrial.orientation(1),task.thistrial.orientationJitter(1),stimulus.displayOrientation(1)));
...
elseif (task.thistrial.thisseg == 1) 
  % put up text for which task the subject should do
...
if task.thistrial.gotResponse < 1
  ...
  if task.thistrial.whichButton == corrButt
    ...
    stimulus.s{task.thistrial.cue} = doStaircase('update',stimulus.s{task.thistrial.cue},1);
  else
    % incorrect
    ...
    stimulus.s{task.thistrial.cue} = doStaircase('update',stimulus.s{task.thistrial.cue},0);
  end
end
...
stimulus.text(1) = mglText('Axis');
stimulus.text(2) = mglText('Remember');
...
if ~isempty(stimfile) && isfield(stimfile,'stimulus') && iscell(stimfile.stimulus.s) && (length(stimfile.stimulus.s) == 2)
  % use the last staircase value
  [initThreshold1 s] = doStaircase('testValue',stimfile.stimulus.s{1});
  [initThreshold2 s] = doStaircase('testValue',stimfile.stimulus.s{2});
else
  % no existing staircase
  initThreshold1 = 15;
  initThreshold2 = 15;
end
disp(sprintf('Initial threshold for working memory: %0.2f non-working memory: %0.2f',initThreshold1,initThreshold2));

% initialize the staircase
stimulus.s{1} = doStaircase('init','upDown','nup=1','ndown=3','initialThreshold',initThreshold1,'initialStepsize=3','nTrials=40');
stimulus.s{2} = doStaircase('init','upDown','nup=1','ndown=3','initialThreshold',initThreshold2,'initialStepsize=3','nTrials=40');
