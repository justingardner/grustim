...
%% Call
% offsetRetinotopy('attend=1','displayName=fMRIProjFlex','bars=1','fixedRandom=1','stimulusPeriod=24','stepsPerCycle',48,'blanks',3,'doEyeCalib=0');

%%
function myscreen = offsetRetinotopy(varargin)

% evaluate the arguments
eval(evalargs(varargin,0,0,{'attend','wedges','rings','bars','barsTask','barsFix','barsTaskEasy','barsTaskDefault','barAngle','barContrast','elementAngle','direction','dutyCycle','stepsPerCycle','stimulusPeriod','numCycles','doEyeCalib','initialHalfCycle','volumesPerCycle','displayName','easyFixTask','dispText','barWidth','barSweepExtent','elementSize','barStepsMatchElementSize','synchToVolEachCycle','blanks','fixedRandom','yOffset','xOffset','imageWidth','imageHeight'}));
...
% attention position
if ieNotDefined('attend'),attend=0; end % attend to center (1= left, 2 = right)

stimulus.attend = attend;
stimulus.attOpts = [0 -5 5];
...
% if ~easyFixTask
  ...
  fixStimulus.interTime = 0.6; %0.35
  fixStimulus.stairUsePest = 1;

  fixStimulus.pos = [stimulus.attOpts(stimulus.attend+1) 0];
...
% if stimulusType ~= 4
  fixTaskNum = 1;
  stimulusTaskNum = 2;
  [task{fixTaskNum}, myscreen] = gruFixStairInitTask_afmap(myscreen);
% else
