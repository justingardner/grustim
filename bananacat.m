...
%   number of distractors will appear. After a brief stimulus presentation
%   all of the items will be blanked by dynamic masks (100 ms) before
%   turning black. The observer must then saccade to the target location
%   and the next trial follows. (A power-wheel mode allows for testing
%   without saccades).
%
%   Performance is evaluated as % correct at a specific eccentricity and
%   display length. 
%
...
% add arguments later
ecc=0;sz=0;plots=0;debug=0;training=0;powerwheel=0;group='';scan=0;len=0;distractors=0;genTex=0;noeye=0;gray=0;
getArgs(varargin,{'ecc=5','sz=5','distractors=3','len=250','plots=0','gray=0','noeye=0','debug=0','training=0','powerwheel=0','group=objects','scan=0','genTex=1'});
stimulus.scan = scan;
stimulus.ecc = ecc;
stimulus.sz = sz;
...
clear plots debug training powerwheel ecc sz group scan distractors len noeye
...
task{1}{1}.seglen = [0.250 0.750 stimulus.len/1000 0.100 2.000 0.500];
...
task{1}{1}.parameter.len = stimulus.len;
task{1}{1}.parameter.dist = stimulus.dist;
task{1}{1}.parameter.gray = stimulus.gray;

task{1}{1}.parameter.matchStyle = [0];
task{1}{1}.parameter.matchCat = [1];
...
% get the rotation pattern and positions
stimulus.live.total = stimulus.dist+1;
stimulus.live.angle = 0:pi*2/stimulus.live.total:(pi*2-0.1);
stimulus.live.angle = mod(stimulus.live.angle+rand*pi*2,pi*2);
% the target will be the first position, distractors at the others
% get the xy pos
stimulus.live.x = stimulus.ecc*cos(stimulus.live.angle);
stimulus.live.y = stimulus.ecc*sin(stimulus.live.angle);

disp(sprintf('(bananacat) Trial (%i): %s%s hidden among %i %s%s at ecc %i, size %1.1f, len %i ms',...
...
if any(task.thistrial.thisseg==[stimulus.seg.resp stimulus.seg.iti])
...
elseif task.thistrial.thisseg==stimulus.seg.cue
    stimulus.live.cue = 1;
    stimulus.live.fix = 0;
elseif task.thistrial.thisseg==stimulus.seg.mask
    stimulus.live.mask = 1;
end

for i = 1:2
    if stimulus.live.mask, break; end
...
if stimulus.live.mask
    mglBltTexture([stimulus.mask{randi(length(stimulus.mask))} stimulus.mask{randi(length(stimulus.mask))} stimulus.mask{randi(length(stimulus.mask))} stimulus.mask{randi(length(stimulus.mask))}],...
        [stimulus.live.x' stimulus.live.y'],0,0,randi(360,1,4));
